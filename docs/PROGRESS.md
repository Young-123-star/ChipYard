...
## 当前阶段（2026-07-10）
**核心业务与生产化基线稳定，安全巡检首版已实现完成，待部署验证。**

### 2026-07-07 至 2026-07-09 基线补录
- 数据初始化：新增 Excel 导入模块和生产迁移 `V4__import_unique_constraints.sql`。
- 台账能力：楼栋、楼层、房间、床位、居住人、入住/退宿、费用、抄表和维修等 13 类台账支持 CSV 导出。
- 配置能力：新增数据字典与自定义房间设施，生产迁移为 `V5__dict_management.sql`。
- 交付链路：新增 GitHub Actions CI（前端构建 + 后端测试）和手动生产部署工作流。

### 2026-07-10 巡检管理首版（已完成）
- 设计：`docs/superpowers/specs/2026-07-10-巡检管理-design.md`。
- 新增巡检计划和巡检任务两表，生产迁移为 `V6__inspection_management.sql`；巡检项复用可维护的数据字典。
- 支持计划维护/启停、按日期手工派发、逐项执行、异常说明、任务级整改确认和取消待执行任务。
- 同一计划同一日期数据库唯一；任务保存计划、对象和巡检项快照，计划修改不影响历史任务。
- 前端新增「安全巡检 / 巡检管理」单页双标签，巡检对象支持楼栋、楼层和房间。
- 验证：后端全量 `mvn test` 151 tests passed；前端 `npm run build` 通过。

### 下一步候选（不发散）
- 先通过 GitHub Actions 手动部署生产，确认 Flyway V6 成功，再完成页面冒烟。
- 巡检稳定后按原路线图进入访客管理；RBAC、自动派发、移动巡检和附件继续后置。

> 设计文档：`docs/superpowers/specs/2026-06-11-宿舍管理系统DEMO-design.md`

## 当前阶段（2026-07-06）
**项目 DEMO 已部署到服务器，当前进入生产化收敛与小闭环业务增强阶段。** README 仅临时记录过 2026-07-03 的部署安全化改造；README 暂不作为开发流水账，后续仍以本文件恢复上下文。

### 已部署/版本基线
- 本地 `main` 跟踪 `origin/main`；本次维修联动提交后需推送远程并部署。
...
### 2026-07-06 维修工单联动房间状态（已完成）
- 行为口径：新建维修工单即把关联房间置为「维修中」；受理中的工单继续保持房间「维修中」。
- 完成/取消工单后，若该房间已无待处理或受理中的维修工单，则按床位占用情况恢复房间状态：满员恢复「已满」，否则恢复「空闲」。
