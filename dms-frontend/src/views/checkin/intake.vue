<template>
  <DataView title="入住意向单列表" :total="total">
    <template #filters>
      ...
      <el-select v-model="query.source" placeholder="来源：全部" clearable style="width: 120px" @change="search">
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="tagTypeOf(INTAKE_STATUS, row.status)" size="small" round>{{ labelOf(INTAKE_STATUS, row.status) }}</el-tag>
        </template>
      </el-table-column>
  ...
  <!-- 手工新建 -->
  <el-dialog v-model="createVisible" title="手工新建入住意向单" width="480px">
...
if (route.query.status !== undefined && INTAKE_STATUS.some(s => s.value === initStatus)) query.status = initStatus
